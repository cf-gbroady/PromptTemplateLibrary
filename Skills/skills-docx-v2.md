...
- **Body Text**: Calibri, 10pt, 1.15 line spacing
- **Captions**: Calibri, 8pt, italic, color #666666
...
#### Convert DOCX to PDF with Formatting Preservation
    ...
    def convert_docx_to_pdf(docx_path, pdf_path):
        """
        Convert DOCX to PDF with enhanced formatting preservation using reportlab.
        """
        try:
            # Load the DOCX file
            ...
            for para in doc.paragraphs:
                if para.text.strip():  # Ignore empty paragraphs
                    # Create formatted text preserving bold/italic
                    formatted_text = ""
                    for run in para.runs:
                        text = run.text.replace('&', '&amp;').replace('<', '&lt;').replace('>', '&gt;')
                        if run.bold and run.italic:
                            formatted_text += f"<b><i>{text}</i></b>"
                        elif run.bold:
                    ...
                    if para.alignment:
                        custom_style = ParagraphStyle(
                            'CustomStyle',
                            parent=body_style,
                            alignment=para.alignment  # Preserve original alignment
                        )
            ...
            for table in doc.tables:
                ...
                for i, row in enumerate(table.rows):
                    ...
                    for j, cell in enumerate(row.cells):
                        # Handle cell text with formatting
                    ...
                    # Calculate column widths on first row
                    if i == 0:
                        col_widths = [2*inch] * len(row.cells)  # Default width
            ...
            print(f"Error: Permission denied accessing '{docx_path}' or writing to '{pdf_path}'.")
            return None
    ...
    if __name__ == "__main__":
        ...
        if os.path.exists(docx_path):
            ...
            print(f"Input file '{docx_path}' does not exist.")
...
## Integration with Other Formats - Convert to HTML - Excel (XLSX to DOCX) - Markdown to DOCX
